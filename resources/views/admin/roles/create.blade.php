                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('New role') }}</li>
                        </ol>
                    </nav>
                </div>
                <div class="ms-auto">
            ...
            <div class="row">
                <div class="mx-auto">
                    <form action="{{ route('admin.roles.store') }}" method="POST" class="row g-3">
                        @csrf
                        <h6 class="mb-0 text-uppercase">{{ __('New role') }}</h6>
                        <hr/>
                        ...
                        <div class="card border-top border-0 border-4 border-primary">
                            <div class="card-header">
                                <div class="card-title d-flex align-items-center">
                                    ...
                                    <h5 class="mb-0 text-primary">{{ __('New role') }}</h5>
                                </div>
                            </div>
                            <div class="card-body row">
                                <x-forms.input-text idName="inputRoleName" name="name" label="{{ __('Role Name') }}" type="text" divClass="col-lg-3 col-md-6" value="{{ old('name') }}" helpText="{{ __('Role name.') }}" />
                            </div>
                        </div>
                        <!-- ./card -->
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    @foreach( \App\Models\Permission::orderBy('title')->get() as $permission )
                                        ...
                                        @endphp
                                        <div class="form-check">
                                            <input class="form-check-input permission-checkbox" type="checkbox" value="{{ $permission->id }}" name="permissions[]" id="inputAssignPermission-{{ $permission->id }}" @if( is_array( old('permissions') ) ) @if( in_array( $permission->id, old('permissions') ) ) checked="checked" @endif @endif>
                                            <label class="form-check-label" for="inputAssignPermission-{{ $permission->id }}">{{ $permission->title }}</label>
                                        </div>
                                    @endforeach
                                    </div>
                                </div>
